...
def getLastMatrix(file):
    p = list(csv.reader(file, delimiter='\n'))
    num_cells = len([r.strip() for r in p[0][0].split(',')]) -1
...
def create_db_and_tables(config_folder):
    ...
    first_replay_matrix_schema = """
        CREATE TABLE rat_first_replay_matrix
                     ( config       INTEGER,
                       rat          INTEGER,
                       first_replay_matrix array,
                       mean         REAL,
                       std          REAL,
                       total_connection REAL, 
                       PRIMARY KEY ( config, rat )
    ...
    cursor.execute(first_replay_matrix_schema)
...
def merge_first_replay_matrix_from_all_rats(config_folder, config_number):
    num_rats = len(glob.glob(config_folder + "r*-First_Replay_Matrix.csv"))
    rats  = np.arange(num_rats, dtype=np.uint8)
    file_name = config_folder + "r{}-First_Replay_Matrix.csv"
    replay_matrices = []
    means = []
    stds = []
    totals = []
    ...
    return pd.DataFrame({
        'config'          : config_number,
        'rat'             : rats,
        'first_replay_matrix'   : replay_matrices,
        'mean'            : means,
        'std'             : stds,
        'total_connection'             : totals,
    })
...
def process_config(base_folder, config, sample_rate):
    ...
    all_first_replay_matrix = merge_first_replay_matrix_from_all_rats(config_folder, config_number)
    ...
    # store first replay matrix in database
    all_first_replay_matrix.to_sql('rat_first_replay_matrix', db, if_exists='append', index=False)

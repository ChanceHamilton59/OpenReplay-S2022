...
public class ReplayMatrix {
	...
	public ArrayList<Line2D> wall_seg = new ArrayList<>();
	...
	public void setWall_seg(LinkedList<ObstacleBiases.WallData> walls) {
		for(ObstacleBiases.WallData wall : walls) {
			this.wall_seg.add(new Line2D.Float(wall.wall.x1, wall.wall.y1, wall.wall.x2, wall.wall.y2));
		}
	}
	...
	public void update(float replay_matrix_update_lr) {
		...
		for (int i = 1; i < num_current_cells; i++) {
			...
			// Gets x,y of current pc
			var current_pc_center_x = placeCellBins[1].xs[i];
			var current_pc_center_y = placeCellBins[1].ys[i];
			...
			for (int j = 1; j < num_old_cells; j++) {
				...
				// Get x,y of old pc
				var old_pc_center_x = placeCellBins[0].xs[j];
				var old_pc_center_y = placeCellBins[0].ys[j];
				Line2D motion_seg = new Line2D.Float(current_pc_center_x, current_pc_center_y, (float)(old_pc_center_x), (float)(old_pc_center_y));
				for(Line2D wall : this.wall_seg){
					if (!(motion_seg.intersectsLine(wall))) {
	...
	public void writeFirstRMatrix(int episode, boolean append) {
		...
		// Creates the file name structure
		var file = prefix + "First_Replay_Matrix.csv";

...
import com.github.biorobaw.scs_models.openreplay_f2021.model.modules.f_unseen.UnseenPath;

public class ReplayModelUnseen extends ReplayModel {
...
//    public int[] path_to_follow = new int[]{ 0,1,2,3,4,5,6,7,2,2,2,2 };
    public String file_path_unseen;
    public UnseenPath path_to_follow;
    ...
    public ReplayModelUnseen(XML xml) {
        super(xml);
        ...
        // Unseen Paths Input
        file_path_unseen = xml.getAttribute("file_path_unseen");
        path_to_follow = new UnseenPath(file_path_unseen);
...
//		optimal_action_dist = softmax;
        ...
        var c = Integer.parseInt(Experiment.get().getGlobal("cycle").toString());
        var trial = Integer.parseInt(Experiment.get().getGlobal("episode").toString());
        if (trial == 0){
            chosenAction = path_to_follow.path1.get(c % path_to_follow.path1.size());
        }
        else if(trial == 1){
            chosenAction = path_to_follow.path2.get(c % path_to_follow.path2.size());
...
//        super.endEpisode();
        ...
        if ((episode-1) % (freq_replay_matrix_clear *num_starting_positions) == 0 && episode<num_episodes-1 && episode>2) {
...
//                cell_activation_indexs[1] = test_pc[0];
                ...
                replay_reward = 100*getReward(x_prime, y_prime);

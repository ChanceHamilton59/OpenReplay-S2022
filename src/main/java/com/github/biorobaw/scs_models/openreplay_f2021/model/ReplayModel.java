...
public class ReplayModel extends Subject {
    ...
    public int num_starting_positions;
    ...
    public ReplayModel(XML xml) {
        ...
        num_starting_positions = xml.getIntAttribute("numStartingPositions");
        ...
        replay_memory = xml.getIntAttribute("replay_memory");
...
//      SimulationControl.setPause(true);
    ...
    public long runModel() {
        ...
        if (oldStateValue != null) {
            ...
            //rmatrix.setPcs_current(pcs);
            if(cycles==2){
                var walls = obstacle_biases.walls;
                rmatrix.setWall_seg(walls);
            }
            else if (cycles > 2) {
...
//      optimal_action_dist = softmax;
    ...
    public void newEpisode() {
        ...
        if (episode == num_starting_positions+2) {
            rmatrix.writeFirstRMatrix(episode, false);
        }
    }
    ...
    public void endEpisode() {
        ...
        // Preforms replay propigations for offline learning if feeder was found
        ...
        if ((episode-1) % (replay_memory *num_starting_positions) == 0 && episode<num_episodes-1 && episode>2) {
    ...
    public boolean replayPropagation(boolean new_path_flag) {
        ...
        if (new_path_flag) {
            ...
            boolean has_valid_PC = (rmatrix.possible_starting_PCs.size()>0);
            if (starting_replay_location.equals("random") && has_valid_PC){
            ...
            // TODO: add a stochastic sampling for selecting the starting PC for replay propagation
            else if(starting_replay_location.equals("stochastic") && has_valid_PC){
                start_pc_index = rmatrix.possible_starting_PCs.get(rn.nextInt(rmatrix.possible_starting_PCs.size()));
            }

            else if(starting_replay_location.equals("starting_location") && has_valid_PC){
                ...
                var pos = positions.get(generator.nextInt(positions.size()));
                var active = pc_bins[0].getActive_pcs((float) pos[0], (float) pos[1]).ids;
                start_pc_index = rmatrix.possible_starting_PCs.get(active[generator.nextInt(active.length)]);
            }
...
//        if (cell_activation_indexs[1] == -1 || replay_cycle > max_replay_path) {
            replay_flag = false;
            new_path_flag = true;
//            writeReplayEvent(replay_path_cells);
        ...
        if (replay_flag) {
            ...
            // Detects if action crosses an obsticle
            var action_selected = (int) Math.round((theta / dt)) % numActions;
            if (action_selected < 0) {
                action_selected += numActions;
            }
            var possible_actions = crossWall(x1,x2,obstacle_biases.walls);
            // Checks to see if the action crossed a wall
            if(!possible_actions.contains(action_selected) && possible_actions.size() > 0){
                // go opposite direction
//                action_selected = (action_selected + 4)%8;
...
//                continue_replay = false;
                ...
                // choose random action that does not cross the wall
                action_selected = possible_actions.get(rn.nextInt(possible_actions.size()));
//                System.out.println(action_selected);
            }
            else if (possible_actions.size() == 0){
                continue_replay = false;
            }
            ...
            if (continue_replay){
                ...
                // TODO: see if this selects the next PC as one of the activated PCS after the motion

//                cell_activation_indexs[1] = test_pc[0];
                // calculate activity for current state
                ...
                replay_reward = 1*getReward(x_prime, y_prime);
...
//            cell_activation_indexs = rmatrix.replayEvent(cell_activation_indexs[1],propagation_type);
            var test_pc = pc_bins[0].getActive_pcs((float) x_prime, (float) y_prime).ids;
            cell_activation_indexs = rmatrix.replayEvent(test_pc[0],propagation_type);
...
//                writeReplayEvent(replay_path_cells);
    ...
    ArrayList<Integer> crossWall(float start_x, float start_y, LinkedList<ObstacleBiases.WallData> walls){
        Set<Integer> union = new HashSet<Integer>();
        ArrayList<Integer> possible_action = new ArrayList<>();
        ArrayList<Line2D> motion_seg = new ArrayList<>();
        ArrayList<Line2D> wall_seg = new ArrayList<>();
        for (Integer i = 0; i < 8; i++ ){
            float tita = (float) (2 * Math.PI / numActions * i);
            var x_prime = start_x + max_move_distance * Math.cos(tita);
            var y_prime = start_y + max_move_distance * Math.sin(tita);
            motion_seg.add(new Line2D.Float(start_x, start_y, (float)(x_prime), (float)(y_prime)));
        }
        for(ObstacleBiases.WallData wall : walls){
            wall_seg.add(new Line2D.Float(wall.wall.x1, wall.wall.y1, wall.wall.x2, wall.wall.y2));
        }
        int action = 0;
        for(Line2D motion : motion_seg){
            boolean crosses = false;
            for(Line2D wall : wall_seg){
                if (motion.intersectsLine(wall)) {
                    crosses = true;
                    break;
                }
            }
